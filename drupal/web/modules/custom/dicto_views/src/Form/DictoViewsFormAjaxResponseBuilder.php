...
use Drupal\dicto_views\Render\MainContent\JsonRenderer;
 ...
 */
class DictoViewsFormAjaxResponseBuilder extends FormAjaxResponseBuilder {
  ...
  /**
   * @var \Drupal\dicto_views\Render\MainContent\JsonRenderer
   */
   ...
   *   The current route match.
   */
  public function __construct(MainContentRendererInterface $ajax_renderer, RouteMatchInterface $route_match, JsonRenderer $jsonRenderer) {
  ...
  public function buildResponse(Request $request, array $form, FormStateInterface $form_state, array $commands) {
    ...
    if ($result instanceof AjaxResponse) {
      ...
      // if request is AMP you probably want a json result anyway
      if ($request->query->has('__amp_source_origin')) {
        return $this->jsonRenderer->renderResponse($result, $request, $this->routeMatch);
      }
